            ...
            for (;;) {
                ...
                if (inPacket.verifyChecksum()) {
                    if (!connected) {
                        if (inPacket.isSYN()) {
                            ...
                            responder.socket.send(new DatagramPacket(outBytes, outBytes.length, dstAddress,
                                    Utils.router ? Utils.router_port : dstPort));
                            continue;
                        } else if (!inPacket.isACK() || inPacket.getSeqNum() != initialSeqNum)
                            continue;
                    }
                    ...
                    if (inPacket.isFIN() && !sentFinAck) {
                        logger.info("Received FIN packet " + inPacket.getSeqNum() + ". Sending FIN + ACK to ("
                                + dstAddress + ", " + dstPort + ")");
                        ...
                        logger.info("Received ACK after sending FIN + ACK. Closing Connection to (" + dstAddress + ", "
                                + dstPort + ")");
                        stopTimer();
                        break;
                        ...
                        for (CodeVectorElement c : codeVector) {
                            ...
                            if (remaining < 0) // case for when the file is less
                                               // than 1 block in size
                            ...
                            data = new BigInteger(dataBytes); // TODO: better
                                                              // variable names
